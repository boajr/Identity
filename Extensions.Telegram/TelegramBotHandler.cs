...
namespace Boa.Identity.Telegram
{
        ...
        where TUser : class
    {
        ...
        public async Task<bool> HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
        {
            ...
            {
                "RESETPWD" => await ResetPassword(botClient, update.Message.ReplyToMessage, update.Message, cancellationToken),
                _ => false,
            };
        }
        ...
        private async Task<bool> ResetPassword(ITelegramBotClient botClient, Message request, Message response, CancellationToken cancellationToken)
        {
            ...
            await botClient.DeleteMessageAsync(response.Chat.Id, request.MessageId, cancellationToken);
            await botClient.DeleteMessageAsync(response.Chat.Id, response.MessageId, cancellationToken);
            ...
            TUser? user = await _userStore.FindByTelegramIdAsync(response.From!.Id, cancellationToken);
            if (user == null)
            {
